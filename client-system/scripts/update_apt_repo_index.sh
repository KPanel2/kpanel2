...
for suite in "${SUITES[@]}"; do
	...
	for arch in "${ARCHES[@]}"; do
		...
		# dpkg-scanpackages emits absolute Filename values when run from a realpath
		# repo root; apt then requests debian.kumpeapps.com//home/... and gets 404.
		sed -i "s|^Filename: ${REPO_ROOT}/|Filename: |" "${out_dir}/Packages"
		sed -i 's|^Filename: .*/pool/|Filename: pool/|' "${out_dir}/Packages"
